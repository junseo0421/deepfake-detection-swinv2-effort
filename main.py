...
def parse_option():
    ...
    parser.add_argument('--cfg',
                        default='/scratch/x3438a06/KJS/codes/deep_fake_swin/configs/swinv2/swinv2_large_patch4_window12to24_192to384_22kto1k_ft.yaml',
    ...
    parser.add_argument('--data_path', type=str,
                        default='/scratch/x3438a06/dataset', help='path to dataset')
    ...
    parser.add_argument('--output', default='/scratch/x3438a06/KJS/outputs/swin2', type=str, metavar='PATH',
    ...
    parser.add_argument('--train_csv', default='/scratch/x3438a06/dataset/metadata_train.csv', type=str, help='train csv path')
    parser.add_argument('--val_csv', default='/scratch/x3438a06/dataset/metadata_valid.csv', type=str, help='val csv path')
    parser.add_argument('--num_frames', type=int, default=5, help='frames to sample from video')
    parser.add_argument('--mode', type=str, default='train', choices=['train', 'infer'])
    parser.add_argument('--ckpt', type=str, default='', help='(infer) checkpoint path .pth')
    ## effort
...
def main(config, args):
    ...
    if config.MODEL.RESUME:
        ...
        logger.info(f"[Resume] Acc@1 {acc1:.2f}% | loss {loss:.4f} | AUC {auc:.6f}")
        if config.EVAL_MODE:
            return
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        acc1, loss, auc = validate(config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}% | AUC {auc:.6f}")
        max_accuracy = max(max_accuracy, acc1)
        logger.info(f'Max accuracy: {max_accuracy:.2f}%')
        if not np.isnan(auc):
            max_auc = max(max_auc, auc)
        logger.info(f'Max auc: {max_auc:.6f}')
...
def all_gather_list(obj):
    """DDP 환경에서 파이썬 객체(list 등)를 모든 rank로부터 gather."""
...
def broadcast_scalar(x: float, src: int = 0) -> float:
    """rank0의 스칼라 값을 모든 rank에 전달."""
    if not (dist.is_available() and dist.is_initialized()):
        return float(x)
...
def validate(config, data_loader, model):
    ...
    for idx, (images, target) in enumerate(data_loader):
        ...
        acc1_meter.update(acc1.item(), n=1)

        probs_fake = torch.softmax(output.float(), dim=1)[:, 1]  # [B]
        y_true_local.extend(target.detach().cpu().tolist())
        y_score_local.extend(probs_fake.detach().cpu().tolist())
    ...
    gathered_true = all_gather_list(y_true_local)
    gathered_score = all_gather_list(y_score_local)

    if get_rank() == 0:
        y_true = [v for part in gathered_true for v in part]
        y_score = [v for part in gathered_score for v in part]

        # 한 클래스만 존재하면 roc_auc_score가 에러 -> 방어
        try:
            if len(set(y_true)) < 2:
                auc = float("nan")
            else:
    ...
    auc = broadcast_scalar(auc, src=0)

    logger.info(f' * Acc@1 {acc1_meter.avg:.3f} | ROC-AUC {auc:.6f}')
...
if __name__ == '__main__':
    ...
    if args.mode == "train":
        ...
        # infer 모드 (baseline submission 루프 유지)
        # 모델 구성
        ...
        if args.use_effort:
            if args.effort_variant == "attn":
                apply_effort_swin_last_stage_attn_only(
                    model,
                    k_residual=args.k_residual,
                    freeze_others=False,  # True 일 시에는 residual + head만 학습 / False 일 시에는 fine tuning 가능
                    train_svd_bias=args.train_svd_bias
                )
            else:
                apply_effort_swin_last_stage_attn_and_mlp(
        ...
        model.load_state_dict(state, strict=True)  # 가능하면 strict=True 사용
        ...
        NUM_FRAMES = args.num_frames  # infer에서도 동일 옵션 쓰기

        files = sorted([p for p in TEST_DIR.iterdir() if p.is_file()])
        results: Dict[str, float] = {}

        for file_path in tqdm(files, desc="Processing"):
            out = preprocess_one(file_path, NUM_FRAMES, TARGET_SIZE)
            if out.error:
            ...
            if out.imgs:
                probs = infer_fake_probs(out.imgs, model, DEVICE, tfm)
